...
// Id doesn't exist
func GetPackageById(id int64) (v *Package, err error) {
	o := orm.NewOrm()
	v = &Package{Id: id}
	if err = o.QueryTable(new(Package)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func GetAllPackage(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64, deviceID int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Package)).Filter("Device", deviceID)
	// query k=v
	for k, v := range query {
...
func DeletePackage(id int64) (err error) {
	o := orm.NewOrm()
	v := Package{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
